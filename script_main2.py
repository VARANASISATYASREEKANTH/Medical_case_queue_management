...
class MedicalCaseAssignmentEnv(gym.Env):
    def __init__(self, patient_data_file=r"C:\Users\SREEKANTHVS\Desktop\raga_ai\patient_data.csv"):
        ...
        self.patient_data = pd.read_csv(patient_data_file)
    ...
    def reset(self):
        # Load next patient details from CSV
        if self.current_patient_index >= len(self.patient_data):
            self.current_patient_index = 0  # Reset index if end of dataset is reached
        
        patient = self.patient_data.iloc[self.current_patient_index]
        self.current_patient_index += 1
        
        self.patient_age = patient["age"] / 100  # Normalize age
        self.patient_gender = 1 if patient["gender"].lower() == "female" else 0
        self.medical_history = patient["medical_history"]
        self.case_urgency = patient["urgency"]
        self.preferred_doctor = int(patient["preferred_doctor"])
...
evaluate_agent(env)
env.save_results("C:/Users/SREEKANTHVS/Desktop/raga_ai/metrics_evaluation_results.csv")